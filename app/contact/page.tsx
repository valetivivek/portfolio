    ...
    <main className="container-page">
        <section className="py-12 md:py-16">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <Link className="btn-primary" href="mailto:vivekvaleti7053@gmail.com" aria-label="Email">
              Email
            </Link>
          ...
          <form onSubmit={onSubmit} className="mt-8 max-w-xl space-y-4">
            <div>
              ...
              <input id="name" name="name" value={name} onChange={(e) => setName(e.target.value)} required minLength={2} maxLength={80} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 shadow-subtle focus-visible:ring-primary-600" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-900" htmlFor="email">Email</label>
              <input id="email" name="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 shadow-subtle focus-visible:ring-primary-600" />
            </div>
            ...
            <div>
              ...
              <textarea id="message" name="message" value={message} onChange={(e) => setMessage(e.target.value)} required minLength={10} maxLength={5000} rows={6} className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 shadow-subtle focus-visible:ring-primary-600" />
            </div>
            <div className="flex items-center gap-3">
              <button type="submit" className="btn-primary" disabled={isPending}>{isPending ? "Sending..." : "Send message"}</button>
              <a className="btn-ghost" href={`mailto:vivekvaleti7053@gmail.com?subject=${encodeURIComponent("Portfolio contact")}&body=${encodeURIComponent(message)}`}>Open email client</a>
            </div>
